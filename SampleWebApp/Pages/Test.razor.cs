...
public partial class Test
{
    ...
    public Test(IJSRuntime jsRuntime, ILogger<Test> logger)
    {
        ...
        _tests = new Dictionary<string, Func<Task>>
        {
            // ------------------------
            // ---- Editor Methods ----
            // ------------------------
    ...
    private async Task RunTest()
    {
        foreach (var testName in _tests.Keys)
        {
            ...
            if (_tests.TryGetValue(testName, out var testMethod))
                await testMethod();
        }
    }
    ...
    private async Task AddCommand()
    {
        ...
        // Act
        await _editor.AddCommand((int)KeyMod.CtrlCmd | (int)KeyCode.Enter, _ =>
        {
            runIsCalled = true;
        });
    ...
    {
        // Arrange
        const string expected = "SetValue test value line 1\ntest line 2";
    ...
    private async Task SetScrollPosition()
    {
        ...
        // Assert
        await Task.Delay(ValueUpdateDelayMs);
        var actualTop = await _editor.GetScrollTop();
        var actualLeft = await _editor.GetScrollLeft();
        AssertEquals(actualTop, expected.ScrollTop);
        AssertEquals(actualLeft, expected.ScrollLeft);
    }

    private async Task GetScrolledVisiblePosition()
    {
        // Arrange
        var scrollPosition = new NewScrollPosition
        {
            ScrollLeft = 50,
            ScrollTop = 75
        };
        await _editor.SetValue(LoremIpsum);
        await _editor.SetScrollPosition(scrollPosition, ScrollType.Immediate);
        await Task.Delay(ValueUpdateDelayMs);
    ...
    private void AssertEquals<T>(T actual, T expected, string testName = "", [CallerMemberName] string caller = "")
        where T : IComparable
    {
        testName = string.IsNullOrWhiteSpace(testName)
            ? caller
            : $"{caller}.{testName}";
